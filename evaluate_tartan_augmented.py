...
from csvo.overexpoture_blur import blur_and_overexpose
...
def video_iterator(imagedir, ext=".png", preload=True):
    ...
    for imfile in sorted(imfiles)[::STRIDE]:
        ...
        if 'image_left_plana' in imfile:
            ...
            pathl = imfile.replace("image_left_overexpoture", "SDL_frames_low_rate").replace("_left.png", '.npy')
        else:
            pathr = imfile.replace("image_left", "SDR_frames_low_rate").replace("_left.png", '.npy')
            pathl = imfile.replace("image_left", "SDL_frames_low_rate").replace("_left.png", '.npy')
...
def run(imagedir, cfg, network, viz=False, ablation="RGB", sdEncoderPath="sdEncoder.pth"):
    slam = CSVO(cfg, network, ht=480, wd=640, viz=viz, ablation=ablation, sdEncoderPath=sdEncoderPath, augmented=True)
        ...
        for j in range(trials):
            ...
            if plot:
                ...
                plot_trajectory((traj_est, tstamps), (traj_ref, tstamps), f"TartanAir {scene_name.replace('_', ' ')} Trial #{j+1} (ATE: {ate_score:.03f})",
                                f"trajectory_plots/{path}/TartanAir_{scene_name}_Trial{j+1:02d}.pdf", align=True, correct_scale=True)

            if save:
                Path("saved_trajectories"+'/'+path).mkdir(exist_ok=True)
                save_trajectory_tum_format((traj_est, tstamps), f"saved_trajectories/{path}/TartanAir_{scene_name}_Trial{j+1:02d}.txt")
            print(j,"done")
        # return
