...
class CSVO:
    def __init__(self, cfg, network, ht=320, wd=640, viz=False, ablation="RGB", sdEncoderPath=None, isTianmouc=False, augmented=False):
        self.ablation=ablation
        self.augmented = augmented

        self.cfg = cfg
        self.load_weights(network, sdEncoderPath=sdEncoderPath)
        ...
        self.viewer = None
        print("MASK_RGB", self.cfg.MASK_RGB)
        print("GRADIENT_BIASED", self.cfg.GRADIENT_BIAS)
        if viz:
            self.start_viewer()

    def load_weights(self, network, sdEncoderPath=None):
        # load network from checkpoint file
        if isinstance(network, str):
            from collections import OrderedDict
            state_dict = torch.load(network)
            if 'model_state_dict' in state_dict.keys() and 'optimizer_state_dict' in state_dict.keys():
            ...
            with autocast(enabled=True):
                ...
                try:
                    self.net, (delta, weight, _) = \
                        self.network.module.update(self.net, ctx, corr, None, self.ii, self.jj, self.kk)
                except:
                    self.net, (delta, weight, _) = \
    ...
    def __call__(self, tstamp, image,sdr, sdl, intrinsics):
        """ track new frame """
        ...
        if self.cfg.MASK_RGB:
            image[True] = 0
        
        with autocast(enabled=self.cfg.MIXED_PRECISION):
            try:
                fmap, gmap, imap, patches, _, clr = \
                    self.network.module.patchify(image,sdl=sdl, sdr=sdr,
                        patches_per_image=self.cfg.PATCHES_PER_FRAME, 
                        gradient_bias=self.cfg.GRADIENT_BIAS, 
                        return_color=True, isTianmouc=self.isTianmouc)
            except:
                fmap, gmap, imap, patches, _, clr = \
                    self.network.patchify(image,sdl=sdl, sdr=sdr,
                        patches_per_image=self.cfg.PATCHES_PER_FRAME, 
                        gradient_bias=self.cfg.GRADIENT_BIAS, 
                        return_color=True, isTianmouc=self.isTianmouc)
