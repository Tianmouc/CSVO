...
class CorrLayer(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        if ctx.dropout < 1:
            perm = torch.rand(len(ii), device=mycuda) < ctx.dropout
            coords = coords[:,perm]
            grad = grad[:,perm]
            ii = ii[perm]
            jj = jj[perm]
...
def patchify(net, coords, radius, mode='bilinear', rank=None):
    """ extract patches """
    if rank!=None:
        mycuda=rank
    else:
    ...
    coords = coords.to(mycuda)
    #print('[coorelation.py]',torch.max(coords),torch.min(coords),radius)
    #print('[coorelation.py]',net.shape)
    #print('[coorelation.py]',coords.device,net.device)
    patches = PatchLayer.apply(net, coords, radius)
    #print('[coorelation.py]',coords.device,net.device,patches.device)
    # print(mycuda)
    if mode == 'bilinear':
