...
class CholeskySolver(torch.autograd.Function):
    ...
    def backward(ctx, grad_x):
        ...
        dH = -torch.matmul(xs, dz.transpose(-1,-2).contiguous())
...
def block_matmul(A, B):
    ...
    A = A.permute(0, 1, 3, 2, 4).contiguous().reshape(b, n1*p1, m1*q1)
    B = B.permute(0, 1, 3, 2, 4).contiguous().reshape(b, n2*p2, m2*q2)
    return torch.matmul(A, B).reshape(b, n1, p1, m2, q2).permute(0, 1, 3, 2, 4).contiguous()
...
def block_solve(A, B, ep=1.0, lm=1e-4):
    ...
    return X.reshape(b, n1, p1, m2, q2).permute(0, 1, 3, 2, 4).contiguous()
...
def BA(poses, patches, intrinsics, targets, weights, lmbda, ii, jj, kk, bounds, ep=100.0, PRINT=False, fixedp=1, structure_only=False):
    ...
    wJiT = (weights * Ji).transpose(2,3).contiguous()
    wJjT = (weights * Jj).transpose(2,3).contiguous()
    wJzT = (weights * Jz).transpose(2,3).contiguous()
    ...
    B = safe_scatter_add_mat(Bii, ii, ii, n, n).view(b, n, n, 6, 6).contiguous() + \
        safe_scatter_add_mat(Bij, ii, jj, n, n).view(b, n, n, 6, 6).contiguous() + \
        safe_scatter_add_mat(Bji, jj, ii, n, n).view(b, n, n, 6, 6).contiguous() + \
        safe_scatter_add_mat(Bjj, jj, jj, n, n).view(b, n, n, 6, 6).contiguous()

    E = safe_scatter_add_mat(Eik, ii, kk, n, m).view(b, n, m, 6, 1).contiguous() + \
        safe_scatter_add_mat(Ejk, jj, kk, n, m).view(b, n, m, 6, 1).contiguous() 

    C = safe_scatter_add_vec(torch.matmul(wJzT, Jz), kk, m)

    v = safe_scatter_add_vec(vi, ii, n).view(b, n, 1, 6, 1).contiguous() + \
        safe_scatter_add_vec(vj, jj, n).view(b, n, 1, 6, 1).contiguous()
    ...
    if structure_only or n == 0:
        dZ = (Q * w).view(b, -1, 1, 1).contiguous()

    else:
        S = B - block_matmul(EQ, E.permute(0,2,1,4,3).contiguous())
        y = v - block_matmul(EQ, w.unsqueeze(dim=2))
        dX = block_solve(S, y, ep=ep, lm=1e-4)

        dZ = Q * (w - block_matmul(E.permute(0,2,1,4,3).contiguous(), dX).squeeze(dim=-1))
        dX = dX.view(b, -1, 6).contiguous()
        dZ = dZ.view(b, -1, 1, 1).contiguous()
