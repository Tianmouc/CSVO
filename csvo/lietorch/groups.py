...
class LieGroup:
    ...
    def apply_op(cls, op, x, y=None):
        ...
        return data.view(out_shape + (-1,)).contiguous()
    ...
    def matrix(self):
        ...
        I = I.view([1] * (len(self.data.shape) - 1) + [4, 4]).contiguous()
        return self.__class__(self.data[...,None,:]).act(I).transpose(-1,-2).contiguous()

    def translation(self):
        """ extract translation component """
        p = torch.as_tensor([0.0, 0.0, 0.0, 1.0], dtype=self.dtype, device=self.device)
        p = p.view([1] * (len(self.data.shape) - 1) + [4,]).contiguous()
        return self.apply_op(Act4, self.data, p)
    ...
    def view(self, dims):
        data_reshaped = self.data.view(dims + (self.embedded_dim,)).contiguous()
