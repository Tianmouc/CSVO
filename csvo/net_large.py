...
class DualModalityFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        out1 = out1 + x1
        out2 = out2 + x2
        # output = self.featurefuser(out1, out2)
        # print(out1.shape, out2.shape)
        return out1, out2
...
class DualModalityFusionStack(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DualModalityFusionStack, self).__init__()
...
def rgb_or_sd(events):
    ...
    return res, events[1:]
# from shift_shuffle_fusion import DualModalityFusion

autocast = torch.cuda.amp.autocast
import matplotlib.pyplot as plt

import sys
sys.path.append('/data_4T/zzx/RepViT/model')
...
class Patchifier(nn.Module):
    def __init__(self, patch_size=3, ablation='RGB', use_pretrained_sd=False):
        ...
        # self.fnet = Encoder3D(output_dim=128, norm_fn='instance')
        ...
        # for param in self.fnet.parameters(): #把fnet冻结
        ...
        import os
        # if os.path.exists(sdEncoderPath):
        #     model_data = torch.load(sdEncoderPath, map_location=torch.device('cpu'))
        #     print("Pretrained SD encoder's path:", sdEncoderPath)
        #     filtered_state_dict = {k.replace("module.", ''): v for k, v in model_data.items()}
        #     self.fnetSD.load_state_dict(filtered_state_dict)
        #     filtered_state_dict = {k.replace("module.", ''): v for k, v in model_data.items()}
        #     self.fnetSD2.load_state_dict(filtered_state_dict)
        ...
        #     model_data = torch.load('encoder_sd_inet.pth', map_location=torch.device('cpu'))
        #     filtered_state_dict = {k.replace("module.", ''): v for k, v in model_data.items()}
        #     self.inetSD.load_state_dict(filtered_state_dict)
        #     filtered_state_dict = {k.replace("module.", ''): v for k, v in model_data.items()}
        #     self.inetSD2.load_state_dict(filtered_state_dict)
        #     print("Pretrained SD encoder loaded!")
        #     model_data = torch.load('encoder_sd_fnet.pth', map_location=torch.device('cpu'))
        #     filtered_state_dict = {k.replace("module.", ''): v for k, v in model_data.items()}
        #     self.fnetSD.load_state_dict(filtered_state_dict)
        #     filtered_state_dict = {k.replace("module.", ''): v for k, v in model_data.items()}
        #     self.fnetSD2.load_state_dict(filtered_state_dict)
        ...
        if ablation != 'dpvo':
            self.fuserf = DualModalityFusionStack(128,128)
            self.fuseri = DualModalityFusionStack(DIM, DIM)
    ...
    def __image_gradient(self, images):
        ...
        return g
    def __sd_gradient(self, images):
        # gray = ((images + 0.5) * (255.0 / 2)).sum(dim=2)
        gray = images.squeeze(2)  # 去掉单通道维度，形状变为 1*b*h*w
        dx = gray[...,:-1,1:] - gray[...,:-1,:-1]
        dy = gray[...,1:,:-1] - gray[...,:-1,:-1]
        g = torch.sqrt(dx**2 + dy**2)
        g = F.avg_pool2d(g, 4, 4)
    ...
    def forward(self, images, patches_per_image=80, disps=None, gradient_bias=False, return_color=False, time_surface0=None, time_surface1=None, sdr=None, sdl = None, events=None, rank=None, isTianmouc=None, augmented=False):
        ...
        n = images.shape[1]
        # print("n =",n)
        if 'plana' in self.ablation.lower() or 'taught' in self.ablation.lower() :
            # 此处资源消耗略大，但也可以接受，可以多卡跑
            # image_map = self.fnet(images) / 4.0
            # sds1 = torch.cat((sdr, sdl), dim=2)
            # sdmap1 = self.fnetSD(sds1) / 4.0
            # fmap1= self.fuserf(image_map[0], sdmap1[0])
            # fmap = fmap1.unsqueeze(0)
            # # print(fmap1.shape)
            # # else:
            # # print("correct2")
            # sds2 = torch.cat((sdr, sdl), dim=2)
            # fmap2 = self.fnetSD2(sds2) / 4.0
            
            # for i in range(images.shape[1]):
            #     # print(images[0, i])
            #     if torch.all(images[0, i] == -0.5):
            #         # print("SD Only")
            #         fmap[0][i] = fmap2[0][i]
            #     # else:
            #     #     print("RGB+SD")
            ...
            # sds1 = torch.cat((sdr, sdl), dim=2)
            # sdmap1 = self.inetSD(sds1) / 4.0
            # imap1= self.fuseri(image_map[0], sdmap1[0])
            # imap = imap1.unsqueeze(0)
            # # print(fmap1.shape)
            # # else:
            # # print("correct2")
            # sds2 = torch.cat((sdr, sdl), dim=2)
            # imap2 = self.inetSD2(sds2) / 4.0
            ...
            for i in range(images.shape[1]):
                ...
                if (torch.all(images[0, i] == -0.5) and isTianmouc) or (random.randint(0,5) == 1 and not isTianmouc):
                    # print("SD Only")
                    zero_index.append(i)
            # print("len of zero_index:", len(zero_index))
            if len(zero_index) == n:
                ...
                # images_nozero = torch.gather(images, 1, index_array.unsqueeze(0).unsqueeze(0).unsqueeze(2).unsqueeze(3))
                images_nozero = torch.cat([images[0][i].unsqueeze(0) for i in range(n) if i not in index_array], dim=0).unsqueeze(0)
                # print("image_nozero shape:",images_nozero.shape)
        ...
        if gradient_bias:
            ...
            # print("coods shape:", coords.shape)
        ...
        # save_normalized_tensor_as_image(path, gmap)
        if return_color:
