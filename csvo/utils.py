...
def coords_grid(b, n, h, w, **kwargs):
    ...
    return coords[[1,0]].view(1, 1, 2, h, w).contiguous().repeat(b, n, 1, 1, 1)
...
def coords_grid_with_index(d, **kwargs):
    ...
    y = y.view(1, 1, h, w).contiguous().repeat(b, n, 1, 1)
    x = x.view(1, 1, h, w).contiguous().repeat(b, n, 1, 1)

    coords = torch.stack([x, y, d], dim=2)
    index = torch.arange(0, n, dtype=torch.float, **kwargs)
    index = index.view(1, n, 1, 1, 1).contiguous().repeat(b, 1, 1, h, w)
...
def pyramidify(fmap, lvls=[1]):
    ...
    pyramid = []
    for lvl in lvls:
        gmap =  F.avg_pool2d(fmap.view(b*n, c, h, w).contiguous(), lvl, stride=lvl)
        pyramid += [ gmap.view(b, n, c, h//lvl, w//lvl).contiguous() ]
